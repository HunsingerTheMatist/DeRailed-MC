# Fires while a resource item is being used. Run as the player
#
...
execute as @e[type=marker,distance=..10,tag=dr_resource,tag=!dr_init] run function derailed:item/init_marker

scoreboard players operation #curr_player_id dr_arg = @s dr_player_id
tag @e[type=marker,distance=..10,tag=dr_resource] remove dr_current
execute as @e[type=marker,distance=..10,tag=dr_resource] if score @s dr_player_id = #curr_player_id dr_arg run tag @s add dr_current
...
execute at @n[type=marker,distance=..10,tag=dr_resource,tag=dr_current] align xyz positioned ~0.5 ~ ~0.5 run function derailed:item/try_place

kill @e[type=marker,distance=..10,tag=dr_resource,tag=dr_current]
