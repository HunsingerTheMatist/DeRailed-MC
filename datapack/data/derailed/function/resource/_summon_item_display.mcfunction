...
execute if score #curr_resource dr_arg = $Wood dr_const run \
    summon minecraft:item_display ~ ~ ~ {item: {id:"oak_log",    components: {item_model:"derailed:resource/wood"}}, transformation: {left_rotation:[0,0,0,1], right_rotation:[0,0,0,1], scale:[0.8,0.8,0.8], translation:[0,-0.125,0]}, Tags:["dr_resource"]}
execute if score #curr_resource dr_arg = $Iron dr_const run \
    summon minecraft:item_display ~ ~ ~ {item: {id:"iron_block", components: {item_model:"derailed:resource/iron"}}, transformation: {left_rotation:[0,0,0,1], right_rotation:[0,0,0,1], scale:[0.9,0.9,0.9], translation:[0,-0.05,0]}, Tags:["dr_resource"]}
execute if score #curr_resource dr_arg = $Rail dr_const run \
    summon minecraft:item_display ~ ~ ~ {item: {id:"rail",       components: {item_model:"derailed:resource/rail"}}, transformation: {left_rotation:[0,0,0,1], right_rotation:[0,0,0,1], scale:[1.2,1.2,1.2], translation:[0,0.1,0]}, Tags:["dr_resource"]}
...
execute if score #use_darker_tint dr_temp matches 0 run \
